...
client.on("message", async(message) =>
	{
		...
		if(message.content.startsWith("!rt")){
			try {
				...
				let playermode
				playermode = message.content.split(" ")[1];
				const recentplay = await Recentplay(apikey, playername, modeconvert(playermode));
					if(recentplay == 0){
				...
				const acc = calcAccuracy(parseInt(recentplay.count300), parseInt(recentplay.count100), parseInt(recentplay.countmiss));
				...
				const ifFCacc = calcAccuracy(ifFC300, ifFC100, 0, playermode);
